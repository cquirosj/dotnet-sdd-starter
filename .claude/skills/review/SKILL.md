...
- Domain objects are plain C# — no ASP.NET Core attributes, no framework dependencies.
- Dependencies flow inward: controller → service → domain. Never the reverse.
- No circular dependencies between namespaces.
- Expected domain failures return `Result<T>.Failure(...)`, not a thrown exception.

### 2. Naming and Placement
- Acceptance tests end `AcceptanceTests` and live in `tests/<Project>.Tests/Acceptance/`.
- Unit/service tests end `Tests` and live alongside (or in a folder mirroring) the code they test.
...
### 3. Test Quality
- Acceptance tests go through the REST API via `WebApplicationFactory<Program>` — no direct service or repository calls.
- Assertions use concrete values from the spec, not vague checks like `ShouldNotBeNull()` or `ShouldBeGreaterThan(0)`.
...
- No unused `using` directives, dead code, or commented-out blocks.

### 6. Spec Traceability
- Every rule in the Example Mapping spec has a corresponding nested test class.
- Every example in the spec has a corresponding `[Fact]`/`[Theory]` case.
...
modified file with its responsibility and architectural layer, the new behaviour it
